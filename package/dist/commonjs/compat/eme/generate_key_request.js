...
function patchInitData(initData) {
    log_1.default.info("DRM", "Trying to move CENC PSSH from init data at the end of it.");
    ...
    while (offset < initData.length) {
        if (initData.length < offset + 8 ||
            ...
            log_1.default.warn("DRM", "unrecognized initialization data. Cannot patch it.");
        ...
        if (initData[offset + 12] === 0x10 &&
            ...
            log_1.default.info("DRM", "CENC PSSH found with version", version);
            if (version === undefined) {
                log_1.default.warn("DRM", "could not read version of CENC PSSH");
            }
                ...
                log_1.default.warn("DRM", "cenc version 1 encountered, " + "removing every other cenc pssh box.");
                concatenatedCencs = currentPSSH;
                foundCencV1 = true;
            }
            else {
                log_1.default.warn("DRM", "filtering out cenc pssh box with wrong version", version);
            }
        }
        else {
...
function generateKeyRequest(session, initializationDataType, initializationData) {
    log_1.default.debug("DRM", "Calling generateRequest on the MediaKeySession");
    var patchedInit;
    try {
    ...
    return session.generateRequest(initDataType, patchedInit).catch(function (error) {
        ...
        log_1.default.warn("DRM", "error while calling `generateRequest` with an empty " +
