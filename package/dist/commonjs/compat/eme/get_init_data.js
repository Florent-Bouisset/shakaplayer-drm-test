...
function getInitializationDataValues(initData) {
    ...
    while (offset < initData.length) {
        if (initData.length < offset + 8 ||
            ...
            log_1.default.warn("DRM", "Unrecognized initialization data. Use as is.");
        ...
        if (isPSSHAlreadyEncountered(result, currentItem)) {
            ...
            log_1.default.warn("DRM", "Duplicated PSSH found in initialization data, removing it.");
        }
        else {
...
function getInitData(encryptedEvent) {
    var initData = encryptedEvent.initData, initDataType = encryptedEvent.initDataType, forceSessionRecreation = encryptedEvent.forceSessionRecreation;
    if ((0, is_null_or_undefined_1.default)(initData)) {
        log_1.default.warn("DRM", "No init data found on media encrypted event.");
        return null;
    }
    var initDataBytes = new Uint8Array(initData);
    var values = getInitializationDataValues(initDataBytes);
    return { type: initDataType, values: values, forceSessionRecreation: forceSessionRecreation };
}
