...
function RepresentationStream(_a, callbacks, parentCancelSignal) {
    ...
    log_1.default.debug("Stream", "Creating RepresentationStream", {
    ...
    segmentQueue.addEventListener("error", function (err) {
    ...
    segmentQueue.addEventListener("requestRetry", function (payload) {
    ...
    segmentQueue.addEventListener("fullyLoadedSegment", function (segment) {
        segmentSink
    ...
    var segmentsToLoadRef = segmentQueue.resetForContent(content, hasInitSegment);
    canceller.signal.register(function () {
        segmentQueue.stop();
    });
    ...
    function checkStatus() {
        ...
        if (terminateVal === null) {
            ...
            log_1.default.debug("Stream", "Urgent switch, terminate now.", {
                bufferType: bufferType,
                representationBitrate: content.representation.bitrate,
            });
            ...
            var initSegmentRequest = segmentQueue.getRequestedInitSegment();
            var currentSegmentRequest = segmentQueue.getRequestedMediaSegment();
            ...
            if (nextQueue.length === 0 && nextInit === null) {
                log_1.default.debug("Stream", "No request left, terminate", {
                    bufferType: bufferType,
                    representationBitrate: content.representation.bitrate,
                });
                segmentsToLoadRef.finish();
