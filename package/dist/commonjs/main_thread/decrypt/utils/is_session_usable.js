...
function isSessionUsable(loadedSession) {
    ...
    if (keyStatuses.length <= 0) {
        log_1.default.debug("DRM", "isSessionUsable: MediaKeySession given has an empty keyStatuses", {
            sessionId: loadedSession.sessionId,
        });
        return false;
    }
    if ((0, array_includes_1.default)(keyStatuses, "expired")) {
        log_1.default.debug("DRM", "isSessionUsable: MediaKeySession given has an expired key", {
            sessionId: loadedSession.sessionId,
        });
        return false;
    }
    if ((0, array_includes_1.default)(keyStatuses, "internal-error")) {
        log_1.default.debug("DRM", "isSessionUsable: MediaKeySession given has a key with an " + "internal-error", { sessionId: loadedSession.sessionId });
        return false;
    }
    log_1.default.debug("DRM", "isSessionUsable: MediaKeySession is usable", {
        sessionId: loadedSession.sessionId,
    });
    return true;
}
