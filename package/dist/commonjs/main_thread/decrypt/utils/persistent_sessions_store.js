...
var PersistentSessionsStore = /** @class */ (function () {
    ...
    function PersistentSessionsStore(storage) {
        ...
        try {
            ...
            log_1.default.warn("DRM", "Persistent store: Could not get entries from license storage", e instanceof Error ? e : "");
            this.dispose();
        }
    }
    /**
    ...
    PersistentSessionsStore.prototype.add = function (initData, keyIds, session) {
        ...
        if ((0, is_null_or_undefined_1.default)(session) || !(0, is_non_empty_string_1.default)(session.sessionId)) {
            log_1.default.warn("DRM", "Persistent Store: Invalid Persisten Session given.");
            return;
        }
        ...
        if (currentIndex >= 0) {
            ...
            log_1.default.info("DRM", "Persistent Store: Updating session info.", { sessionId: sessionId });
            this._entries.splice(currentIndex, 1);
        }
        else {
            log_1.default.info("DRM", "Persistent Store: Add new session", { sessionId: sessionId });
        }
    ...
    PersistentSessionsStore.prototype.delete = function (sessionId) {
        ...
        if (index === -1) {
            log_1.default.warn("DRM", "Persistent Store: initData to delete not found in persistent store.");
            return;
        }
        var entry = this._entries[index];
        log_1.default.warn("DRM", "Persistent Store: Delete session from persistent store", {
    ...
    PersistentSessionsStore.prototype.deleteOldSessions = function (sessionsToDelete) {
        log_1.default.info("DRM", "Persistent Store: Deleting last sessions.", { sessionsToDelete: sessionsToDelete });
        ...
        if (sessionsToDelete <= this._entries.length) {
            ...
            log_1.default.warn("DRM", "Persistent Store: Asked to remove more information that it contains", sessionsToDelete, this._entries.length);
            this._entries = [];
        }
        this._save();
    };
                ...
                switch (entry.version) {
                    case 4:
                        if (Array.isArray(initData.keyIds) && initData.keyIds.length > 0) {
                                    ...
                                    for (var _b = (e_1 = void 0, __values(entry.keyIds)), _c = _b.next(); !_c.done; _c = _b.next()) {
                                        ...
                                        if (typeof entryKid === "string") {
                                            if (keyIdB64 === entryKid) {
                                                log_1.default.debug("DRM", "Persistent Store: Found wanted kid stored on entry", {
                                                    index: i,
                                                    keyId: keyIdB64,
                                                    sessionId: entry.sessionId,
                                                });
                            ...
                            if (foundCompatible) {
                                log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v4", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                });
                                return { value: i };
                            ...
                            if ((0, are_init_values_compatible_1.default)(formatted, entry.values)) {
                                log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v4 without init data", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                });
                                return { value: i };
                    ...
                    case 3: {
                        var formatted = initData.values.getFormattedValues();
                        if ((0, are_init_values_compatible_1.default)(formatted, entry.values)) {
                            log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v3 - same values", {
                                index: i,
                                sessionId: entry.sessionId,
                            });
                            return { value: i };
                        ...
                        if (entry.initDataHash === concatHash) {
                            try {
                                ...
                                if ((0, are_arrays_of_numbers_equal_1.default)(decodedInitData, concatInitData)) {
                                    log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v2 - same concatenated init data", {
                                        index: i,
                                        sessionId: entry.sessionId,
                                    });
                                    return { value: i };
                                ...
                                log_1.default.warn("DRM", "Persistent Store: Could not decode initialization data.", e instanceof Error ? e : "");
                            }
                        }
                        break;
                    }
                    case 1: {
                        ...
                        if (entry.initDataHash === concatHash) {
                            if (typeof entry.initData.length === "undefined") {
                                ...
                                log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v1 - same hash only", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                    hash: concatHash,
                                });
                                ...
                                log_1.default.debug("DRM", "Persistent Store: Found compatible entry of v1 - same hash and initData", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                    hash: concatHash,
                                });
                    ...
                    default: {
                        ...
                        if (entry.initData === concatHash) {
                            log_1.default.debug("DRM", "Persistent Store: Found compatible entry - same hash only", {
                                index: i,
                                sessionId: entry.sessionId,
                                hash: concatHash,
                            });
    ...
    PersistentSessionsStore.prototype._save = function () {
        try {
            ...
            log_1.default.warn("DRM", "Persistent Store: Could not save MediaKeySession information", err);
        }
    };
