...
var update_manifest_codec_support_1 = require("../../main_thread/init/utils/update_manifest_codec_support");
...
var codec_support_cache_1 = require("./codec_support_cache");
...
var Manifest = /** @class */ (function (_super) {
    ...
    function Manifest(parsedManifest, options) {
        ...
        _this._cachedCodecSupport = new codec_support_cache_1.default([]);
        _this.periods = parsedManifest.periods
            .map(function (parsedPeriod) {
            var period = new period_1.default(parsedPeriod, _this._cachedCodecSupport, representationFilter);
            return period;
        })
     ...
     */
    Manifest.prototype.updateCodecSupport = function (updatedCodecSupportInfo) {
        var e_1, _a;
        if (updatedCodecSupportInfo === void 0) { updatedCodecSupportInfo = []; }
    ...
    Manifest.prototype.updateCodecSupportList = function (cachedCodecSupport) {
     ...
     */
    Manifest.prototype.addRepresentationsToAvoid = function (items) {
        var e_2, _a;
        var updates = [];
        try {
            for (var items_1 = __values(items), items_1_1 = items_1.next(); !items_1_1.done; items_1_1 = items_1.next()) {
                var item = items_1_1.value;
                var period = this.getPeriod(item.period.id);
                if (period === undefined) {
                    continue;
                }
                var adaptation = period.getAdaptation(item.adaptation.id);
                if (adaptation === undefined) {
                    continue;
                }
                var representation = adaptation.getRepresentation(item.representation.id);
     ...
     */
    Manifest.prototype.getCodecsWithUnknownSupport = function () {
        return (0, update_manifest_codec_support_1.getCodecsWithUnknownSupport)(this);
    };
    /**
     * @param {Object} newManifest
...
function updateDeciperability(manifest, isDecipherable) {
    ...
    try {
        for (var _d = __values(manifest.periods), _e = _d.next(); !_e.done; _e = _d.next()) {
            var period = _e.value;
            try {
                for (var _f = (e_5 = void 0, __values(period.getAdaptations())), _g = _f.next(); !_g.done; _g = _f.next()) {
                    var adaptation = _g.value;
                    var hasOnlyUndecipherableRepresentations = true;
                    try {
                        for (var _h = (e_6 = void 0, __values(adaptation.representations)), _j = _h.next(); !_j.done; _j = _h.next()) {
                            ...
                            if (result !== representation.decipherable) {
                                ...
                                log_1.default.debug("manifest", "Decipherability changed for \"".concat(representation.id, "\""), "(".concat(representation.bitrate, ")"), String(representation.decipherable));
                            }
                        }
                    }
