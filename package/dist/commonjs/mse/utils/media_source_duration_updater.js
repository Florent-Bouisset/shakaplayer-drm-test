...
function setMediaSourceDuration(mediaSource, duration, isRealEndKnown) {
    ...
    if (newDuration === mediaSource.duration) {
        ...
        if (maxBufferedEnd < mediaSource.duration) {
            try {
                log_1.default.info("mse", "Updating duration to what is currently buffered", {
                    maxBufferedEnd: maxBufferedEnd,
                });
                mediaSource.duration = maxBufferedEnd;
            }
            catch (err) {
                log_1.default.warn("mse", "Can't update duration on the MediaSource.", err instanceof Error ? err : "");
        ...
        try {
            log_1.default.info("mse", "Updating duration", { newDuration: newDuration });
            mediaSource.duration = newDuration;
            if (mediaSource.readyState === "open" && !isFinite(newDuration)) {
                var maxSeekable = getMaximumLiveSeekablePosition(duration);
                log_1.default.info("mse", "calling `mediaSource.setLiveSeekableRange`", { maxSeekable: maxSeekable });
...
function createMediaSourceOpenReference(mediaSource, cancelSignal) {
    ...
    (0, event_listeners_1.onSourceOpen)(mediaSource, function () {
        log_1.default.debug("mse", "Reacting to MediaSource open in duration updater");
        isMediaSourceOpen.setValueIfChanged(true);
    }, cancelSignal);
    (0, event_listeners_1.onSourceEnded)(mediaSource, function () {
        log_1.default.debug("mse", "Reacting to MediaSource ended in duration updater");
        isMediaSourceOpen.setValueIfChanged(false);
    }, cancelSignal);
    (0, event_listeners_1.onSourceClose)(mediaSource, function () {
        log_1.default.debug("mse", "Reacting to MediaSource close in duration updater");
        isMediaSourceOpen.setValueIfChanged(false);
    }, cancelSignal);
