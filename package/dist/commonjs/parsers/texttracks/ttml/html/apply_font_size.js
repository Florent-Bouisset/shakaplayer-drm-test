...
function applyFontSize(element, fontSize) {
    ...
    if (firstFontSize[2] === "px" || firstFontSize[2] === "em") {
        ...
        if (isNaN(toNum)) {
            log_1.default.warn("ttml", 'could not parse fontSize value "' + firstFontSize[1] + '" into a number');
        }
        else {
            element.style.position = "relative";
            (0, add_class_name_1.default)(element, "proportional-style");
        ...
        log_1.default.warn("ttml", "unhandled fontSize unit", { unit: firstFontSize[2] });
    }
}
