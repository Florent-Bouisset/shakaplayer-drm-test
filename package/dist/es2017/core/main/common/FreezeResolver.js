...
export default class FreezeResolver {
    ...
    _checkForDecipherabilityRelatedFreeze(observation, now) {
        ...
        const { hasUndecipherableData, hasEncryptedData } = haveBuffersUndecipherableData(this._segmentSinksStore);
        ...
        if (shouldHandleDecipherabilityFreeze &&
            ...
            log.warn("Freeze", "we are frozen despite only having decipherable " +
    ...
    _getStrategyIfFlushingFails(freezingPosition) {
        log.warn("Freeze", "A recent flush seemed to have no effect on freeze, checking for transitions");
        ...
        for (const ttype of ["audio", "video"]) {
            ...
            if (previousRepresentationEntry === undefined ||
                ...
                log.debug("Freeze", "Freeze when beginning to play a content, try avoiding this quality");
                toAvoid.push({
