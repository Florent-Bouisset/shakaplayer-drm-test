...
const mediaCapabilitiesProber = {
     ...
     */
    async checkDrmConfiguration(keySystemType, keySystemConfiguration) {
        const eme = getEmeApiImplementation("auto");
        if (eme === null) {
            const error = new Error("EME not supported in current environment");
            throw error;
        }
        const mksa = await eme.requestMediaKeySystemAccess(keySystemType, keySystemConfiguration);
        if (!canRelyOnRequestMediaKeySystemAccess(keySystemType)) {
            try {
                ...
                log.debug("MediaCapabilitiesProber", "KeySystemAccess was granted but it is not usable");
                return Promise.reject(new Error("Failed to generare a license-request with this keySystem: " +
     ...
     */
    async getStatusForHDCP(hdcpVersion) {
     ...
     */
    async getDecodingCapabilities(mediaConfig) {
        const config = {
     ...
     * @returns {Promise.<string>}
     */
    async getDisplayCapabilities(displayConfig) {
        const config = { display: displayConfig };
        let matchMediaSupported;
        try {
