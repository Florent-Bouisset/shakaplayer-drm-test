 ...
 * @returns {Promise}
 */
export default async function probeHDCPPolicy(hdcpVersion) {
    ...
    if (!("getStatusForPolicy" in mediaKeys)) {
        log.error("MediaCapabilitiesProber", "`MediaKeys.prototype.getStatusForPolicy` API is not available");
        throw new Error("MediaKeys.prototype.getStatusForPolicy` API is not available");
    }
    const result = await mediaKeys.getStatusForPolicy(policy);
    if (result === "usable") {
        return "Supported";
    }
    else {
