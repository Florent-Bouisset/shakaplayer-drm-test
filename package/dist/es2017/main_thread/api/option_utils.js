...
function parseLoadVideoOptions(options) {
    ...
    let startAt;
    const { DEFAULT_AUTO_PLAY, DEFAULT_CODEC_SWITCHING_BEHAVIOR, DEFAULT_ENABLE_FAST_SWITCHING, DEFAULT_TEXT_TRACK_MODE, DEFAULT_AUDIO_TRACKS_NOT_PLAYABLE_BEHAVIOR, DEFAULT_VIDEO_TRACKS_NOT_PLAYABLE_BEHAVIOR, } = config.getCurrent();
    ...
    if (defaultAudioTrackSwitchingMode !== undefined &&
        ...
        log.warn("API", "The `defaultAudioTrackSwitchingMode` loadVideo option must match one of " +
    ...
    if (!arrayIncludes(["continue", "reload"], onCodecSwitch)) {
        log.warn("API", "The `onCodecSwitch` loadVideo option must match one of " +
    ...
    let onAudioTracksNotPlayable = isNullOrUndefined(options.onAudioTracksNotPlayable)
    ...
    if (!arrayIncludes(["continue", "error"], onAudioTracksNotPlayable)) {
        log.warn("API", "The `onAudioTracksNotPlayable` loadVideo option must match one of " +
            "the following string:\n" +
            "- `continue`\n" +
            "- `error`\n" +
            "If badly set, " +
    ...
    let onVideoTracksNotPlayable = isNullOrUndefined(options.onVideoTracksNotPlayable)
    ...
    if (!arrayIncludes(["continue", "error"], onVideoTracksNotPlayable)) {
        log.warn("API", "The `onVideoTracksNotPlayable` loadVideo option must match one of " +
            "the following string:\n" +
            "- `continue`\n" +
            "- `error`\n" +
            "If badly set, " +
