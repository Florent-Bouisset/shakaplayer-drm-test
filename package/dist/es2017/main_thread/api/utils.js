...
export function constructPlayerStateReference(initializer, mediaElement, playbackObserver, isDirectFile, cancelSignal) {
    ...
    initializer.addEventListener("loaded", () => {
        if (playerStateRef.getValue() === "LOADING" /* PLAYER_STATES.LOADING */) {
            ...
            if (!cancelSignal.isCancelled()) {
                const newState = getLoadedContentState(mediaElement, null, isDirectFile, playerStateRef.getValue());
    ...
    function updateStateIfLoaded(stallRes) {
        ...
        const newState = getLoadedContentState(mediaElement, stallRes, isDirectFile, playerStateRef.getValue());
 ...
 * @returns {string}
 */
export function getLoadedContentState(mediaElement, stalledStatus, isDirectFile, previousState) {
    ...
    if (stalledStatus !== null) {
        ...
        if (previousState === "LOADED" /* PLAYER_STATES.LOADED */ && !canPreloadBeforePlay(isDirectFile)) {
            /**
