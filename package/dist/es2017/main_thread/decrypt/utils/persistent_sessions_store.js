...
export default class PersistentSessionsStore {
    ...
    constructor(storage) {
        ...
        try {
            ...
            log.warn("DRM", "Persistent store: Could not get entries from license storage", e instanceof Error ? e : "");
            this.dispose();
        }
    }
    /**
    ...
    delete(sessionId) {
        ...
        if (index === -1) {
            log.warn("DRM", "Persistent Store: initData to delete not found in persistent store.");
            return;
        }
    ...
    deleteOldSessions(sessionsToDelete) {
        ...
        if (sessionsToDelete <= this._entries.length) {
            ...
            log.warn("DRM", "Persistent Store: Asked to remove more information that it contains", sessionsToDelete, this._entries.length);
            this._entries = [];
        }
        this._save();
    }
            ...
            if (entry.initDataType === initData.type) {
                switch (entry.version) {
                    case 4:
                        if (Array.isArray(initData.keyIds) && initData.keyIds.length > 0) {
                                ...
                                for (const entryKid of entry.keyIds) {
                                    if (typeof entryKid === "string") {
                                        if (keyIdB64 === entryKid) {
                                            log.debug("DRM", "Persistent Store: Found wanted kid stored on entry", {
                                                index: i,
                                                keyId: keyIdB64,
                                                sessionId: entry.sessionId,
                                            });
                            ...
                            if (areInitializationValuesCompatible(formatted, entry.values)) {
                                log.debug("DRM", "Persistent Store: Found compatible entry of v4 without init data", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                });
                                return i;
                    ...
                    case 3: {
                        const formatted = initData.values.getFormattedValues();
                        if (areInitializationValuesCompatible(formatted, entry.values)) {
                            log.debug("DRM", "Persistent Store: Found compatible entry of v3 - same values", {
                                index: i,
                                sessionId: entry.sessionId,
                            });
                            return i;
                        ...
                        if (entry.initDataHash === concatHash) {
                            try {
                                ...
                                if (areArraysOfNumbersEqual(decodedInitData, concatInitData)) {
                                    log.debug("DRM", "Persistent Store: Found compatible entry of v2 - same concatenated init data", {
                                        index: i,
                                        sessionId: entry.sessionId,
                                    });
                                    return i;
                                ...
                                log.warn("DRM", "Persistent Store: Could not decode initialization data.", e instanceof Error ? e : "");
                            }
                        }
                        break;
                    }
                    case 1: {
                        ...
                        if (entry.initDataHash === concatHash) {
                            if (typeof entry.initData.length === "undefined") {
                                ...
                                log.debug("DRM", "Persistent Store: Found compatible entry of v1 - same hash only", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                    hash: concatHash,
                                });
                                ...
                                log.debug("DRM", "Persistent Store: Found compatible entry of v1 - same hash and initData", {
                                    index: i,
                                    sessionId: entry.sessionId,
                                    hash: concatHash,
                                });
                    ...
                    default: {
                        ...
                        if (entry.initData === concatHash) {
                            log.debug("DRM", "Persistent Store: Found compatible entry - same hash only", {
                                index: i,
                                sessionId: entry.sessionId,
                                hash: concatHash,
                            });
    ...
    _save() {
        try {
            ...
            log.warn("DRM", "Persistent Store: Could not save MediaKeySession information", err);
        }
    }
}
/**
