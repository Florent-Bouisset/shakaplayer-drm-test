...
export default class NativeTextDisplayer {
    ...
    pushTextData(infos) {
        ...
        const { start: startTime, end: endTime, data: dataString, type, language, initTimescale, } = chunk;
        ...
        const cues = parseTextTrackToCues(type, dataString, { initTimescale, language }, timestampOffset);
        ...
        if (end <= start) {
            log.warn("text", "Invalid text track appended: ", "the start time is inferior or equal to the end time.");
            return convertToRanges(this._buffered);
        }
        if (cues.length > 0) {
