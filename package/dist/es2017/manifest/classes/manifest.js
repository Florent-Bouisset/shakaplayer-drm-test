...
export default class Manifest extends EventEmitter {
    ...
    constructor(parsedManifest, options) {
        ...
        this.periods = parsedManifest.periods
            .map((parsedPeriod) => {
            const period = new Period(parsedPeriod, this._cachedCodecSupport, representationFilter);
            return period;
        })
        ...
        for (const adaptation of period.getAdaptations()) {
            ...
            for (const representation of adaptation.representations) {
                ...
                if (result !== representation.decipherable) {
                    ...
                    log.debug("manifest", `Decipherability changed for "${representation.id}"`, `(${representation.bitrate})`, String(representation.decipherable));
                }
            }
