        ...
        const generateLogFn = this._currentFormat === "full"
            ? (logMethod, consoleFn) => {
                return (namespace, ...args) => {
                    const now = getMonotonicTimeStamp();
                    return consoleFn(String(now.toFixed(2)), `[${logMethod}]`, namespace + ":", ...args.map((a) => typeof a === "object" && a !== null && !(a instanceof Error)
            ...
            : (_logMethod, consoleFn) => {
                return (namespace, ...args) => {
                    return consoleFn(namespace + ":", ...args.map((a) => typeof a === "object" && a !== null && !(a instanceof Error)
                        ? formatContextObject(a)
                        : a));
                };
            };
    ...
    for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
            ...
            if (val instanceof Error) {
                ret += `${key}="${JSON.stringify(val === null || val === void 0 ? void 0 : val.toString())}"`;
            }
            else {
